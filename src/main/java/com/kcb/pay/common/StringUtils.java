...
public class StringUtils {
	...
	public static boolean isSameArray(String[] src, String[] desc) {
		...
		if (descLength != srcLength) {
			return false;
		} else {

			for (int i = 0; i < srcLength; i++) {
				if (!contain(src[i], desc)) {
					return false;
				}
			}
			return true;
	...
	private static boolean contain(String str, String[] desc) {
		for (int i = 0; i < desc.length; i++) {
			if (desc[i].equals(str)) {
				return true;
			}
		}
		return false;
...
//		Integer[] array1 = {32,34,33};
//		Integer[] array2 = {26,32,34,33,3};
//		List<Integer> list = compare(array1,array2);
//		for (Integer integer : list) {
//			System.out.println(integer);
//		}
		String str = "1, 2, 3";
		boolean status = str.contains("1, 2");
		if(status){
...
//		System.out.println(has);
	 ...
	 * <code>char</code>s to be represented. If you are needing to support full
	 * I18N of your applications consider using {@link #repeat(String, int)}
	 * instead.
	 * </p>
	 *
	 * @param repeat
	...
	/**
	 * 【三大运营商最新号段 合作版】 移动号段： 134 135 136 137 138 139 147(上网卡) 150 151 152 157
	 * 158 159 178 182 183 184 187 188 联通号段： 130 131 132 145 155 156 171 175 176
	 * 185 186 电信号段： 133 149 153 173 177 180 181 189 虚拟运营商： 170 卫星通信： 1349
	 */
	...
	public static boolean isPASSWORD(String str) {
		boolean re = str != null && str.length() >= 6 ? true : false;
		return re && str.length() <= 20 ? true : false;
	}

	/**
	 * @Title: isValidPassword @Description:
	 *         TODO(密码必须大于6个字符，测试仅判空) @param @param newPass @param @return
	 *         设定文件 @return boolean 返回类型 @throws
	 */
	public static boolean isValidPassword(String newPass) {
		boolean ps = newPass != null && newPass.length() > 5 && newPass.length() < 20;
		return ps;
	}

	/**
	 * @Title: endWith @Description: TODO(这里用一句话描述这个方法的作用) @param @param
	 *         tempVal @param @param string @param @return 设定文件 @return boolean
	 *         返回类型 @throws
	 */
	public static boolean endWith(String tempVal, String string) {
		return tempVal.endsWith(string);
	}

	/**
	 * @Title: substringBefore @Description: TODO(这里用一句话描述这个方法的作用) @param @param
	 *         tempVal @param @param string @param @return 设定文件 @return Object
	 *         返回类型 @throws
	 */
	public static String substringBefore(String tempVal, String string) {
		int index = tempVal.indexOf(string);
		if (index < 0) {
			return tempVal;
		}
		return tempVal.substring(0, index);
	}
	...
	/*
	 * public static boolean Sql(String str) { String inj_str =
	 ...
	 * inj_stra.length; i++) { if (str.indexOf(inj_stra[i]) >= 0) { return true;
	 * } } return false; }
	 */
	...
	/**
	 * 获取客户端请求头的flag标记,区分不同直播图片
	 *
	 * @param request
	 * @return
	 */
	public static String getFlag(HttpServletRequest request) {
